...
class RefundInstallmentOrder implements ShouldQueue   // ShouldQueue 代表异步任务
{
  ...
  {
    // 如果商品订单支付方式不是分期付款、订单未支付、订单退款状态不是退款中，则直接返回
    ...
    foreach ($installment->items as $item) {
      ...
      try {
        ...
        // 假如某个还款计划退款报错，暂时跳过，继续处理下个还款计划退款
        continue;
      }
    }

  // model中 定义判断分期还款是否都成功，并修改状态
