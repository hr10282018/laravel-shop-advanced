...
class InstallmentsController extends Controller
{
  ...
  public function alipayNotify()
  {
    ...
    // 拉起支付时使用的支付订单号由 分期流水号+还款计划编号 组成
    // 因此可以通过支付订单号还原出这笔还款是哪个分期的那个还款计划
    list($no, $sequence) = explode('_', $data->out_trade_no); // list() - 把数组的值赋给list中的参数变量
    // 根据分期流水号查询对应的分期记录，原则不会找不到，这里判断增加健壮性
    if (!$installment = Installment::where('no', $no)->first()) {
      return 'fail';
    }

    // 根据还款计划编号查询对应的还款计划，原则不会找不到，这里判断增加健壮性
    if (!$item = $installment->items()->where('sequence', $sequence)->first()) {
      return 'fail';
    }

    // 如果这个还款计划的支付状态为已支付，则告知此订单已完成，不再执行后续逻辑
    if ($item->paid_at) {
      return app('alipay')->success();
    }
