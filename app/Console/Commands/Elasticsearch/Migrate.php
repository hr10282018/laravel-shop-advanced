...
class Migrate extends Command
{
  ...
  public function handle()
  {
    ...
    foreach ($indices as $indexClass) {
      // 调用类数组的 getAliasName() 获取索引别名
      $aliasName = $indexClass::getAliasName();     // indexClass 就是 'App\Console\Commands\Elasticsearch\Indices',调用静态方法
  ...
  protected function createIndex($aliasName, $indexClass)
  {
    ...
    $this->es->indices()->create([
      ...
      'index' => $aliasName . '_0',
      'body'  => [
        // 调用索引类的 getSettings() 获取索引设置
        'settings'  => $indexClass::getSettings(),
        'mappings'  => [
          // 调用索引类的 getProperties() 获取索引字段
  ...
  {
    // 获取索引信息，返回结构的 key 为索引名称， value为别名
