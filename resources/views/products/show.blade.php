            ...
            @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
              <div class="crowdfunding-info">
                ...
                <div class="progress-info">
                  <span>当前进度：{{ $product->crowdfunding->percent }}%</span>
                  <span>{{ $product->crowdfunding->user_count }}名支持者</span>
                </div>
                <!-- 如果众筹状态是众筹中，则输出提示语 -->
                @if($product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_FUNDING)
                  <div>
                    ...
                    的支持才可成功，
                    <!-- Carbon 对象的 diffForHumans() 方法可以计算出与当前时间的相对时间 -->
                    筹款将在<span class="text-red">{{ $product->crowdfunding->end_at->diffForHumans(now()) }}</span>
                    结束
                  </div>
                @endif
              </div>
...
<script>
  $(document).ready(function() {
    ...
    if ($(this).find('[aria-describedby=""]')) {
      $(this).find('[aria-describedby=""]').css("color", "red");
    }
    ...
    $('.btn-favor').click(function () {
      ...
      axios.post('{{ route('products.favor', ['product' => $product->id]) }}')
        .then(function () { // 请求成功会执行这个回调
          swal('操作成功', '', 'success')
            .then(function () {  // 这里加了一个 then() 方法-重新加载
              location.reload();
            });
    ...
    $('.btn-crowdfunding').click(function(){
      ...
      // 把用户的收货地址以 JSON 形式放入页面，赋给 addresses 变量
