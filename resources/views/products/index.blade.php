            ...
            <div class="col-md-9">
              <div class="form-row">
                ...
                <div class="col-auto category-breadcrumb">
                  ...
                  <!-- 如果当前是通过类目筛选 -->
                  @if($category)
                  <!-- 遍历这个类目的所有祖先类目，我们在模型的访问器已经排好序，因此可以直接使用 -->
        ...
        <div class="filters">
          ...
          <div class="row">
            ...
            <div class="col-9 filter-values">
              ...
              <!-- 调用js定义的 appendFilterToQuery 函数 -->
...
<script>
  // 把控制器传过来的$filters变量转为JSON字符串，赋值给JS的filters变量
  ...
  // 定义函数，用于解析当 Url 的参数，并已 Key-Value 对象形式返回
  function parseSearch(){
    ...
    location.search.substr(1).split('&').forEach(function(str){
      ...
      //将数组的第一个值解码后作为Key，第二个值解码后作为 Value 放到对象中
  ...
  function removeFilterFromQuery(name){
    ...
    searches['filters'].split('|').forEach(function(filter){
      ...
      // 如果当前属性名与要移除的属性名一致，则退出，相当于删除的那一个
      if(result[0] === name){
        return ;
      }
      // 将 filter 放入数组
